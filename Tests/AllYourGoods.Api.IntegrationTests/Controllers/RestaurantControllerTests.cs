...
public class RestaurantControllerTests
{
    ...
    public async Task CreateRestaurant_ValidData_ReturnsCreatedResponseAndStoresInDatabase()
    {
        ...
        Assert.That(returnedDto.Address.HouseNumber, Is.EqualTo(createDto.Address.HouseNumber),
            "Address house number does not match.");
        Assert.That(returnedDto.Address.ZipCode, Is.EqualTo(createDto.Address.ZipCode),
            "Address postal code does not match.");
        Assert.That(returnedDto.Address.City, Is.EqualTo(createDto.Address.City), "Address city does not match.");
        Assert.That(returnedDto.Address.StreetName, Is.EqualTo(createDto.Address.StreetName),
            "Address street does not match.");

        // 5. Assert Banner values
        Assert.That(returnedDto.Banner.Url, Is.EqualTo(createDto.Banner.Url), "Banner URL does not match.");
        Assert.That(returnedDto.Banner.MimeType, Is.EqualTo(createDto.Banner.MimeType),
            "Banner MimeType does not match.");
        ...
        Assert.That(restaurantInDb.PhoneNumber, Is.EqualTo(createDto.PhoneNumber),
            "Stored Phone number does not match.");
        ...
        Assert.That(restaurantInDb.Address.HouseNumber, Is.EqualTo(createDto.Address.HouseNumber),
            "Stored Address house number does not match.");
        Assert.That(restaurantInDb.Address.ZipCode, Is.EqualTo(createDto.Address.ZipCode),
            "Stored Address postal code does not match.");
        Assert.That(restaurantInDb.Address.City, Is.EqualTo(createDto.Address.City),
            "Stored Address city does not match.");
        Assert.That(restaurantInDb.Address.StreetName, Is.EqualTo(createDto.Address.StreetName),
            "Stored Address street does not match.");
        ...
        Assert.That(restaurantInDb.Banner.MimeType, Is.EqualTo(createDto.Banner.MimeType),
            "Stored Banner MimeType does not match.");
        Assert.That(restaurantInDb.Banner.FileSize, Is.EqualTo(createDto.Banner.FileSize),
            "Stored Banner Size does not match.");
        ...
        Assert.That(restaurantInDb.Logo.MimeType, Is.EqualTo(createDto.Logo.MimeType),
            "Stored Logo MimeType does not match.");
        Assert.That(restaurantInDb.Logo.FileSize, Is.EqualTo(createDto.Logo.FileSize),
            "Stored Logo Size does not match.");
    }
    ...
    public async Task GetRestaurants_ValidRequest_ReturnsOkResponseWithPaginatedList()
    {
        ...
        Assert.That(paginatedList.Items.Count, Is.EqualTo(pageSize),
            $"Expected {pageSize} items in the paginated list.");
    ...
    public async Task GetRestaurants_InvalidRequest_ReturnsBadRequest()
    {
        ...
        var response =
            await _client.GetAsync(
                $"/api/restaurant/paginated?pageNumber={invalidPageNumber}&pageSize={invalidPageSize}");
        ...
        Assert.That(errorMessage, Is.EqualTo("Page number and page size must be greater than zero."),
            "Expected specific error message for invalid parameters.");
    }
    ...
    public async Task GetRestaurant_ValidId_ReturnsOkResponse()
    {
        ...
        var returnedDto =
            JsonConvert.DeserializeObject<ResponseRestaurantDto>(await response.Content.ReadAsStringAsync());

        // Assert: Check that the returned restaurant details match the existing restaurant
        Assert.That(returnedDto, Is.Not.Null, "Response body should not be null.");
        Assert.That(returnedDto.Id, Is.EqualTo(existingRestaurantId),
            "Returned restaurant ID does not match the requested ID.");
        Assert.That(returnedDto.Name, Is.EqualTo(_databaseRestaurants.First().Name),
            "Returned restaurant name does not match the expected value.");
    ...
    public async Task GetRestaurant_InvalidId_ReturnsNotFoundResponse()
    {
        ...
        var errorMessage = await response.Content.ReadAsStringAsync();
        Assert.That(errorMessage, Is.EqualTo($"Restaurant with ID = {nonExistentRestaurantId} not found."),
    ...
    public async Task UpdateRestaurant_ValidData_ReturnsOkResponseAndUpdatesInDatabase()
    {
        ...
        Assert.That(returnedDto.PhoneNumber, Is.EqualTo(updateDto.PhoneNumber),
            "Returned PhoneNumber does not match the updated value.");
        Assert.That(returnedDto.AboutUs, Is.EqualTo(updateDto.AboutUs),
            "Returned AboutUs does not match the updated value.");
        Assert.That(returnedDto.Radius, Is.EqualTo(updateDto.Radius),
            "Returned Radius does not match the updated value.");

        // Optional: Verify the update in the database
        using var scope = _factory.Services.CreateScope();
        var context = scope.ServiceProvider.GetRequiredService<ApplicationContext>();

        var restaurantInDb = await context.Restaurants.Include(r => r.Address).Include(r => r.Logo)
            .Include(r => r.Banner)
    ...
    public async Task UpdateRestaurant_InvalidData_ReturnsBadRequest()
    {
        ...
        var errorMessage = await response.Content.ReadAsStringAsync();
        Assert.That(errorMessage, Does.Contain("One or more validation errors occurred."),
        ...
        var restaurants = new List<Restaurant>
        {
            ...
            new()
            {
                ...
                OpeningsTimes = new List<OpeningsTime>
                {
                    new()
                    {
                        Id = Guid.NewGuid(), Opening = new TimeOnly(9, 00), Closing = new TimeOnly(23, 00),
                        Day = Day.Monday
                    },
                    new()
                    {
            ...
            new()
            {
                ...
                OpeningsTimes = new List<OpeningsTime>
                {
                    new()
                    {
                        Id = Guid.NewGuid(), Opening = new TimeOnly(10, 00), Closing = new TimeOnly(22, 00),
                        Day = Day.Monday
                    },
                    new()
                    {
